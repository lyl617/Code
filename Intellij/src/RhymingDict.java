...
public class RhymingDict {
	...
	public static void storeRhyme(DictionaryInterface rhymingDict, String line) {
              String temp_words=getWord(line);//get words
              String temp_rhygroup=getRhymeGroup(line);//get rhy groups
              if(rhymingDict.size()==0){//if no element in rhymingDict
				  MySortedLinkedList temp_sll=new MySortedLinkedList();//new link list
				  temp_sll.add(temp_words);//add to list
				  rhymingDict.put(temp_rhygroup,temp_sll);//put into hashtable
			  }else{
				  for(String key:rhymingDict.getKeys()){//if key exists in rhymingDict hashtable
					if(temp_rhygroup.equals(key)){
						MySortedLinkedList temp_ssl=(MySortedLinkedList)rhymingDict.get(key);
						temp_ssl.add(temp_words);
						return;
					}
				  }
				  MySortedLinkedList temp_sll=new MySortedLinkedList();
				  temp_sll.add(temp_words);
				  rhymingDict.put(temp_rhygroup,temp_sll);
	...
	public static void removeUnrhymables(DictionaryInterface rhymingDict) {
             String [] keys=rhymingDict.getKeys();//get keys in rhymingDict hashtable
             for(String key:keys){
             	MySortedLinkedList temp_sll=(MySortedLinkedList)rhymingDict.get(key);
             	if(temp_sll.size==1){//if key's size is 1 ,remove
